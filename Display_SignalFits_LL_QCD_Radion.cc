...
RooPlot* fitSignal(std::string dirName, TH1D *h, int massNum, std::string mass, int color, TLegend *leg, Params &params, bool kinFit=false)
{
	...
	if (_antitag) w->SaveAs((dirName+"/w_signal_antitag_LL_"+mass+".root").c_str());
	else w->SaveAs((dirName+"/w_signal_LL_"+mass+".root").c_str());
	w->Print();
    }
    return plot;
}
...
int Display_SignalFits_LL_QCD_Radion(std::string dir_preselection="outputs/datacards/",
                       std::string dir_selection="",
                       std::string file_histograms="HH_mX_HH_LL_QCD",
                       int imass=1600,
                       int rebin_factor = 1,
		       bool antitag = false,
                       bool focus=false)
{
	...
	if (antitag)
		{
			std::cout<<"looking for "<<("vh/Signal_mX_antitag_"+masses.at(i)+"HH_LL_QCD_Radion").c_str()<<std::endl;
			h_mX_SR=(TH1D*)file->Get(("vh/Signal_mX_antitag_"+masses.at(i)+"HH_LL_QCD_Radion").c_str());
			h_mX_SR->SetTitle(("m_{X} Peak in Signal MC (m_{X}="+masses.at(i)+" GeV); m_{X} (GeV)").c_str());
		}
	else
		{
			std::cout<<"looking for "<<("Signal_mX_"+masses.at(i)+"_HH_LL_QCD_Radion").c_str()<<std::endl;
			h_mX_SR=(TH1D*)file->Get(("vh/Signal_mX_"+masses.at(i)+"_HH_LL_QCD_Radion").c_str());
			h_mX_SR->SetTitle(("m_{X} Peak in Signal MC (m_{X}="+masses.at(i)+" GeV); m_{X} (GeV)").c_str());
		}
	std::cout<<" FILE OPENED, DONE!: "<<std::endl;
        ...
        double xPad = 0.3;
        TCanvas *c_mX_SR=new TCanvas(("c_mX_SR_"+masses.at(i)).c_str(), ("c_mX_SR_"+masses.at(i)+"HH_LL_QCD").c_str(), 700*(1.-xPad), 700);
